...
int main()
{
    ...
    cout << "Performing the simulation using Metropolis\n________________________________________________\n\n" ;
    while (temp <= 3.1) {
    ...
    cout << "\n\n###########################################\n";
    cout << "Performing the simulation using Gibbs\n________________________________________________\n\n" ;
...
void Averages(int iblk) { //Print results for current block
    ...
    cout << "Block number " << iblk << "/" << nblk << "\r" <<flush;
...
void Print(double t, double h) { //aggiunto mar 19 lug 2022, 11:57:24
    ...
    Ene.open("output.ene."+to_string(h)+str_metro+ ".dat", ios::app);
    Ene << setw(wd) << t << setw(wd) << glob_av[iu]/(double)nblk << setw(wd) << err_u << endl;
    Ene.close();

    Heat.open("output.heat."+to_string(h)+str_metro+ ".dat", ios::app);
    Heat << setw(wd) << t << setw(wd) << glob_av[ic]/(double)nblk << setw(wd) << err_c << endl;
    Heat.close();

    Mag.open("output.mag."+to_string(h)+str_metro+ ".dat", ios::app);
    Mag << setw(wd) << t << setw(wd) <<  glob_av[im]/(double)nblk << setw(wd) << err_u << endl;
    Mag.close();

    Chi.open("output.chi."+to_string(h)+str_metro+ ".dat", ios::app);

...
int main(int argc, char **argv) {
    ...
    // passaggio da riga di comando per la definizione della modalità di lavoro
    if (argc != 2) {
        cerr << "Digitare:\n1-> per studiare il solido;\n2-> per studiare il liquido;\n3-> per studiare il gas;\n"
             << argv[0] << " <mode>\n" << argv[1];
        return -1;
    }
    int mode = atoi(argv[1]); //numero indicante la modalità di funzionamento
    SetUp(mode);
    //Input(file_name); //Inizialization
    int nconf = 1;
...
/*FUNZIONI*/
//Funzione Setup: prende come argomento un numero
// 1-> condurre l'analisi del Solido
// 2-> condurre l'analisi del Liquido
// 3-> condurre l'analisi del Gas
void SetUp(int mode) {
    switch (mode) {
        case 1:
            cout << "Caso Solido" << endl;
            //file_name = "input.solid";
            Input("input.solid");
            break;
        case 2:
            cout << "Caso Liquido" << endl;
            //file_name = "input.liquid";
            Input("input.liquid");
            break;
        case 3:
            cout << "Caso Gas" << endl;
...
// distance i-j in pbc
    ...
    walker[ip] = rho * walker[it] + 16.0 * p / (vol * double(npart));
    return;
}
...
//Potential energy per particle
    Epot << setw(wd) << iblk << setw(wd) << stima_pot << setw(wd) << glob_av[iv] / (double) iblk << setw(wd)
         << err_pot
         << endl;
//Kinetic energy
    Ekin << setw(wd) << iblk << setw(wd) << stima_kin << setw(wd) << glob_av[ik] / (double) iblk << setw(wd)
         << err_kin
         << endl;
//Total energy
    Etot << setw(wd) << iblk << setw(wd) << stima_etot << setw(wd) << glob_av[ie] / (double) iblk << setw(wd)
         << err_etot << endl;
//Temperature
    Temp << setw(wd) << iblk << setw(wd) << stima_temp << setw(wd) << glob_av[it] / (double) iblk << setw(wd)
         << err_temp << endl;
    Pres << setw(wd) << iblk << setw(wd) << stima_pres << setw(wd) << glob_av[ip] / (double) iblk << setw(wd)
         << err_press << endl;
...
void Averages(int iblk, int mode) //Print results for current block: overloading della funzione average,
// in modo che a seconda del tipo di input (gas, liquido, solido) dia ai file finali dei nomi diversi
{
    ofstream Epot, Ekin, Etot, Temp, Pres;
    const int wd = 12;
...
//Potential energy per particle
    Epot << setw(wd) << iblk << setw(wd) << stima_pot << setw(wd) << glob_av[iv] / (double) iblk << setw(wd) << err_pot
         << endl;
//Kinetic energy
    Ekin << setw(wd) << iblk << setw(wd) << stima_kin << setw(wd) << glob_av[ik] / (double) iblk << setw(wd) << err_kin
         << endl;
//Total energy
    Etot << setw(wd) << iblk << setw(wd) << stima_etot << setw(wd) << glob_av[ie] / (double) iblk << setw(wd)
         << err_etot << endl;

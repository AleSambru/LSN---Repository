...
void Averages(int iblk, int mode) //Print results for current block
{
    ...
    //for (int i = 0; i < nbins; i++) {
      //  gr << ((i + 1) * bin_size + i * bin_size) / 2 << setw(wd) << stima_g[i] << setw(wd) << err_g[i] << endl;
        //g_r_blk << ((i + 1) * bin_size + i * bin_size) / 2 << setw(wd) << glob_bin[i] << setw(wd) <<  Error(glob_bin[nblk], glob_bin2[nblk], nblk) << endl;

        //Error(glob_bin[nblk], glob_bin2[nblk], nblk) << endl;
        //cout  << ((i+1)*bin_size + i*bin_size)/2 << setw(wd) << glob_bin[i] << setw(wd)   << endl ;
...
void Print_gr(int mode) {
    ...
    for (int i = 0; i < nbins; i++) {
        gr << ((i + 1) * bin_size + i * bin_size) / 2 << setw(wd) << stima_g[i] << setw(wd) << err_g[i] << endl;
        g_r_blk << ((i + 1) * bin_size + i * bin_size) / 2 << setw(wd) << glob_bin[i] << setw(wd) <<  err_g[i] << endl;

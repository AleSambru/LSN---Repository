...
vector<double> Single_Chi_Quad_Calculator(Random &rnd, bool print) {
    ofstream hist_out;
    hist_out.open("Single_Chi_quad_distr.dat");
    vector<double> chi_quads;
    double chi_quad = 0;
    int M = 100;
    int n_tot = 10000;
// estrmi del jesimo intervallo da analizzare
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < n_tot; j++) {
            ...
            if (ran > i * 1. / M && ran < (i + 1) * 1. / M) {
                count++;
            }
        }
        chi_quads.push_back(pow(count - int (double(n_tot) / M), 2) / int(double(n_tot) / M));
        hist_out << i << "\t" << chi_quads.at(i) << endl;
    }
    //double Chi = accumulate(chi_quads.begin(), chi_quads.end(), 0.);
    //cout << Chi << endl;
    hist_out.close();
    return chi_quads;
};
